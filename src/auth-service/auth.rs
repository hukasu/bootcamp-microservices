...
impl Auth for AuthService {
    async fn sign_in(
        ...
        let result: Option<String> = {
            let guard = self.users_service.lock().expect("Guard to User service.");
            guard.get_user_uuid(req.username, req.password)
        };
        ...
        let session_token: String = {
            let mut guard = self
                .sessions_service
                .lock()
                .expect("Guard to Session service.");
            guard.create_session(&user_uuid)
        };
    ...
    async fn sign_up(
        ...
        let result: Result<(), String> = {
            let mut guard = self.users_service.lock().expect("Guard to Users service.");
            guard.create_user(req.username, req.password)
        };
    ...
    async fn sign_out(
        ...
        // TODO: Delete session using `sessions_service`.
        {
            let mut guard = self
                ...
                .lock()
                .expect("Guard to Session service.");
            guard.delete_session(&req.session_token)
        }
